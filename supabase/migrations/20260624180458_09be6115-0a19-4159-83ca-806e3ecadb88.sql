...
DECLARE
  ...
  -- Relative "activity level" 0..N for each athlete. Actual session inserts
  -- cap at the number of available videos for that position (unique constraint).
...
BEGIN
  ...
  IF p_tier = 'parent' THEN
    ...
    IF v_coach_id IS NOT NULL THEN
      ...
      IF v_team_id IS NOT NULL THEN
        ...
        -- Alex: one completed session per available player video (capped by unique constraint)
  ...
  ELSIF p_tier = 'team' THEN
    ...
    FOR i IN 1..14 LOOP
      ...
      -- Cap real session inserts at the number of available videos for the
      -- position (the dryland_watch_progress unique constraint prevents
      -- duplicate (athlete, video) rows). The relative session_counts[i]
      -- still drives WHICH athletes complete the most: top performers fill
      -- every video, lower ones fill fewer.
      ...
      -- Seed a current streak for the top performers so flame badges appear
