...
DECLARE
  ...
  -- Sessions per athlete this period — varied so the leaderboard is interesting.
  -- index aligns with `names` order (1..14)
...
BEGIN
  ...
  -- Collect all published videos by position so we can rotate through them
  ...
  IF p_tier = 'parent' THEN
    ...
    IF v_coach_id IS NOT NULL THEN
      ...
      IF v_team_id IS NOT NULL THEN
        ...
        -- Alex: 8 completed sessions spanning ~6 weeks across multiple videos
        FOR k IN 1..8 LOOP
          v_vid := v_player_video_ids[1 + ((k - 1) % GREATEST(array_length(v_player_video_ids,1),1))];
    ...
    IF v_team_id IS NOT NULL THEN
      ...
      FOR k IN 1..5 LOOP
        v_vid := v_goalie_video_ids[1 + ((k - 1) % GREATEST(array_length(v_goalie_video_ids,1),1))];
        SELECT total_seconds INTO v_vid_secs FROM public.dryland_videos WHERE id = v_vid;
        INSERT INTO public.dryland_watch_progress (athlete_id, video_id, watched_seconds, last_watched_at, team_id)
        VALUES (v_attendee_id, v_vid, GREATEST((v_vid_secs * 0.95)::int, 60),
  ...
  ELSIF p_tier = 'team' THEN
    ...
    FOR i IN 1..14 LOOP
      ...
      -- Rich dryland history: cycle through multiple videos, spread across ~6 weeks
      v_session_count := session_counts[i];
      FOR j IN 1..v_session_count LOOP
        IF v_dryland_pos = 'goalie' THEN
          v_vid := v_goalie_video_ids[1 + ((j - 1) % GREATEST(array_length(v_goalie_video_ids,1),1))];
        ELSE
          v_vid := v_player_video_ids[1 + ((j - 1) % GREATEST(array_length(v_player_video_ids,1),1))];
        END IF;
        SELECT total_seconds INTO v_vid_secs FROM public.dryland_videos WHERE id = v_vid;
        -- Spread sessions: roughly every 2-3 days back from today, with some jitter per athlete
        v_day_offset := (j - 1) * 2 + ((i + j) % 3);
        INSERT INTO public.dryland_watch_progress (athlete_id, video_id, watched_seconds, last_watched_at, team_id)
      ...
      -- Seed a current streak for the top half so the leaderboard shows flame badges
