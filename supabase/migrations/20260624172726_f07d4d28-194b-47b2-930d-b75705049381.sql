...
BEGIN
  -- Clean prior dev data for this user
  ...
  SELECT id INTO v_vid_player FROM public.dryland_videos WHERE position = 'player' AND is_published = true ORDER BY created_at LIMIT 1;
  SELECT id INTO v_vid_goalie FROM public.dryland_videos WHERE position = 'goalie' AND is_published = true ORDER BY created_at LIMIT 1;
  ...
  IF p_tier = 'parent' THEN
    ...
    -- Find shared coach team (created when 'team' tier is seeded)
    ...
    IF v_coach_id IS NOT NULL THEN
      ...
      IF v_team_id IS NOT NULL THEN
        ...
        -- Dryland progress for Alex this month (3 completed sessions, plus a recent in-progress)
        FOR i IN 1..3 LOOP
          INSERT INTO public.dryland_watch_progress (athlete_id, video_id, watched_seconds, last_watched_at, team_id)
          VALUES (v_attendee_id, v_vid_player, 1700, now() - (i * interval '6 days'), v_team_id);
        END LOOP;
      END IF;
    END IF;

    -- Kid 2: Sam Dev (goalie)
    INSERT INTO public.attendees (owner_id, contact_id, full_name, birthday, position, preferred_position, jersey_number)
    ...
    IF v_team_id IS NOT NULL THEN
      ...
      FOR i IN 1..2 LOOP
        INSERT INTO public.dryland_watch_progress (athlete_id, video_id, watched_seconds, last_watched_at, team_id)
        VALUES (v_attendee_id, v_vid_goalie, 1100, now() - (i * interval '8 days'), v_team_id);
      END LOOP;
    END IF;

    -- A camp registration so /parent/camps has content
  ...
  ELSIF p_tier = 'team' THEN
    -- PRIMARY TEAM: "Lightning U14" — full roster + schedule + past game summary
    INSERT INTO public.teams (coach_id, name, season, age_group)
    VALUES (p_user_id, 'Lightning U14', '2026-27', 'U14')
    RETURNING id INTO v_team_id;
    v_team2_id := v_team_id;

    -- 14 athletes + team_players (linked via athlete_id so leaderboard works)
    FOR i IN 1..14 LOOP
      ...
      -- Dryland watch progress this month (varying counts so leaderboard differentiates)
      FOR j IN 1..(1 + (i % 5)) LOOP
        INSERT INTO public.dryland_watch_progress (athlete_id, video_id, watched_seconds, last_watched_at, team_id)
        VALUES (v_player_id,
                CASE WHEN v_dryland_pos = 'goalie' THEN v_vid_goalie ELSE v_vid_player END,
                1700, now() - ((j * 4 + i) || ' days')::interval, v_team_id);
      END LOOP;
    END LOOP;

    -- SCHEDULE: past game (yesterday, completed) + upcoming events
    ...
    -- Practice plan on the upcoming practice
    ...
    -- Game-day summary content for the past game
    ...
    INSERT INTO public.game_lineups (team_id, event_id, template_name, positions, shared, created_by)
